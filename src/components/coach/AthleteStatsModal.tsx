...
export function AthleteStatsModal({
  ...
  return (
    <ModalDialog label={`${displayName}'s stats history`} onClose={onClose}>
      <div className="modal-header">
        <h2>{displayName}&apos;s Stats History</h2>
        <button type="button" className="ghost modal-close" onClick={onClose} aria-label="Close" data-autofocus>
          <X size={18} aria-hidden="true" />
        </button>
      </div>

      <div className="tabs" role="group" aria-label="Stats sections">
        {TABS.map((t) => (
          <button
            key={t.id}
            type="button"
      ...
      {tab === "recovery" &&
        (loading ? (
          <p className="muted" role="status">Loading stats history...</p>
        ) : error ? (
      ...
      {tab === "prs" &&
        (prsLoading ? (
          <p className="muted" role="status">Loading PRs...</p>
        ) : prsError ? (
          <div className="empty-state">
      ...
      {tab === "adherence" &&
        (adherenceLoading ? (
          <p className="muted" role="status">Loading workout history...</p>
        ) : adherenceError ? (
          <div className="empty-state">
